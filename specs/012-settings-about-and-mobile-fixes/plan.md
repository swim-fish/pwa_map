...
|------|-------------|----------|-----------------|--------------|
| Entry-chunk JS delta gzipped | +1 KiB | **+4.15 KB** | +6 KB | The four slices each contribute: (a) `threeDLockdown.ts` module + the `applyLockdown` filter in `styleBuilder.ts`; (b) the `MapView.svelte` MapLibre options block grew by the type-widened cast pattern (research §R4 — required because `maplibre-gl@^3.6.2` doesn't expose `projection` on its options type); (c) the `SettingsSheet.svelte` About section markup + 9 new i18n strings × 3 locales; (d) the per-layout `@media` blocks in 6 Go-To layout components. Each slice on its own would have stayed within +1 KiB, but the four shipping together aggregate above the plan target. The per-feature delta budget enforced by `scripts/check-bundle-size.js` (+6 KB) is honoured (+4.15 KB ≤ 6 KB). Splitting into four PRs to fit under +1 KiB each was rejected: the slices share the SettingsSheet contrast / i18n test net + the same review-loop gates, so a single PR is cheaper to review and ship without introducing intermediate states with mixed UI affordances. |
